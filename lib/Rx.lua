...
------------------------------------------------------------------------------------------------------------------------
--- Ellypse modification to expose RxLua to the World of Warcraft environment
------------------------------------------------------------------------------------------------------------------------
...
--region Custom Rx integrations for WoW
local Ellyb = Ellyb(...)

--- Creates an observable that will receive a game event
---@param gameEventName string
---@return Observable
function Observable.createFromGameEvent(gameEventName)
    local subject = Subject.create()
    Ellyb.GameEvents.registerCallback(gameEventName, function(...) subject:onNext(...) end)
    return subject
end

--- Creates an observable that will receive a secure function hook
---@param functionName string A global function name
---@return Observable
function Observable.createFromSecureFunctionHook(functionName)
    local subject = Subject.create()
...
---@field text Subject
---@field atlas Subject
---@field modelUnit Subject
local Bindings = {}

local function subjectFrom(widget, method)
    local subject = Subject.create()
...
---@private
Bindings.text = function(widget)
    return subjectFrom(widget, "SetText")
end

---@private
Bindings.atlas = function(widget)
    return subjectFrom(widget, "SetAtlas")
end

---@private
Bindings.modelUnit = function(widget)
    return function(...) widget:SetModelUnit(...) end
end

Bindings.hook = function(widget)
...
Rx.Bindings = Bindings

---@generic T
---@param widget T
---@return RxBindings
function Rx.bindingsFor(widget)
    ...
    for name, binding in pairs(Rx.Bindings) do
        rxBindings[name] = binding(widget)
    end
